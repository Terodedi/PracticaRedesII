...
class Usuario():
    ...
    def problemas_comandos(self):
        ...
        print('Sintaxis correcta: clientetcp.py -s IP -p PUERTO')
...
class Instrucciones():
    ...
    def oack(self, socket):
        recv_size = struct.calcsize('!H')
        if self.timeout != 0:
            recv_size += struct.calcsize('!7sB'+str(len(str(self.timeout)))+'sB')
        if self.size != 512:
            recv_size += struct.calcsize('!7sB' + str(len(str(self.size))) + 'sB')
        oackpack = socket.recv(recv_size)
        print('LLEGA: ', oackpack)
    ...
    def write(self,argumentos,socket):
        ...
        if os.path.isfile(('archivosC/'+argumentos)):
            ...
            print('ERROR, el archivo que quieres enviar no existe')
        Usuario().fin(True)
    ...
    def read(self,argumentos,socket):
        ...
        if not (os.path.isfile(('archivosC/' + nom_arch))):
            socket.send(self.Inicio(1, argumentos, 'octet'))
            if self.size != 512 or self.timeout != 0: # Esperamos OACK para ver que ha entendido el servidor
                self.oack(socket)
            with open(('archivosC/' + nom_arch), "wb") as archivo:
                while True:
                    fragmento = socket.recv(self.size + 4)
                    codigo_operacion = struct.unpack('!H', fragmento[0:2])[0]
                    if codigo_operacion == 5:
